...
(define-test initarg-writer-tests
  ...
  (let ((base-class (find-class 'base-test-class))
    ...
    (is equal '(:BASE-SLOT-2 . (SETF BASE-SLOT-2)) ; Note: reader-only slot, but initarg exists. Writer is nil.
          (com.evocomputing.clos-helpers:initarg-writer-pair base-class 'base-slot-2)
          "INITARG-WRITER-PAIR finds reader-only slot with initarg")
...
(define-test initarg-writer-fn-tests
  ...
  (let ((base-class (find-class 'base-test-class))
    ...
    (false (com.evocomputing.clos-helpers:slot-writer-fn base-class 'base-slot-2) "SLOT-WRITER-FN returns nil for reader-only")
    ...
    (false (com.evocomputing.clos-helpers:initarg-writer-fn-pair simple-class 'slot-b)
           "INITARG-WRITER-FN-PAIR returns nil for slot with no initarg/writer-fn")
    ...
    (let ((pairs (com.evocomputing.clos-helpers:all-initarg-writer-fn-pairs base-class)))
      (is eq 3 (length pairs) "ALL-INITARG-WRITER-FN-PAIRS returns correct number of pairs")
    ...
    (let ((pairs (com.evocomputing.clos-helpers:all-initarg-writer-fn-pairs simple-class)))
       (is eq 1 (length pairs) "ALL-INITARG-WRITER-FN-PAIRS on simple class returns correct number of pairs"))))
